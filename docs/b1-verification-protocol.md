...
deploying reds within **one tick — and the WORST OBSERVED TICK IS 9.9 HOURS (2026-08-06T14:29Z →
2026-08-07T00:25Z), with a typical 50–80 minutes. This sentence claimed "~15 minutes" until 2026-08-10
and "~70 min by day / past 3.5 h overnight" until 2026-08-17 (B9-2, then B9-3); both were means, and
delivery is NOT STATIONARY, so a mean is the wrong statistic for a sentence that says "at most". Two of
the 9.9 hours' constituent runs were cancelled before probing — register #60.** That is a real backstop and it is why the
...
`production-monitor` workflow (PR **#75**) re-asserts all three on a schedule that **requests** `*/30`
and **delivers a WORST OBSERVED BLIND WINDOW of 9.9 hours (2026-08-06) against a typical 50–80 min;
delivery is NON-STATIONARY and no single mean is durable** (B9-3; the "every 15 minutes" written here
until 2026-08-10 was never delivered, and the `~68 / ~113 min` means that replaced it were the
*baseline* figures, superseded 2026-08-17). They stopped
...
| V1 | `/health` returns 200 | machine-verified + **CI-observed** | 2026-08-02 14:06:10 | **PASS** — HTTP 200 | `GET /health` → `200` in 0.43s. Re-asserted since 14:59Z by `production-monitor` (PR #75) at a **requested** `*/30` and a **WORST OBSERVED BLIND WINDOW of 9.9 h (2026-08-06), typical 50–80 min, NON-STATIONARY** — **corrected 2026-08-10 from "every 15 min", which this cell asserted and the scheduler never delivered (B9-2), and again 2026-08-17 off the `~68 / ~113 min` baseline means, which are not what this cell should have been quoting (B9-3)**; first run [30753322454](https://github.com/tornidomaroc-web/knowflow/actions/runs/30753322454) green in 9s. | built from `9e31c22` · observed 2026-08-02T14:06:10Z · **ARTIFACT REAPED** (Railway status: *Removed*) · dep `d7fb32b5` — an 8-hex **deployment-ID prefix, NOT a commit SHA** · deployment live 2026-08-02T14:02Z → 14:59:24Z, so the observation falls inside its lifetime |
...
| **N11** | **Production upload via `/ingest` passes Q3** — **post-merge OBSERVATION, not a merge gate (§3.1)** | **owner-attested** — the raw row was read from the database by the repository owner and pasted verbatim; no agent here ran the query | 2026-08-10 15:43:55Z | **PASS** — all five Q3 conditions | Q3 **adjudicated by `document_id`, not by position**, on `bfd6e9d5-9e95-4739-8579-7fca784f81dc` (`xilvaroth-n11-20260810.md`, 1177 bytes, sha256 `03582C47…98A6`, subject `43387669-8189-4970-96aa-9499464291df` "ZZ N11 PROD CHECK 2026-08-10", **created empty with its id recorded before any file existed in it**): `status='ready'`, `embedding_status='ready'`, `chunk_count=1` = `actual_chunk_rows=1`, `has_markdown=true` (`markdown_len=1175`), `error_message` null. **Owning user is `d6fff4eb-6b98-4750-ac7f-2d5001f233f7` — NOT the `72ebd7b5-dae9-48da-ac53-d58e4533eedf` behind V4's document, see the account-identity finding below this table.** **CORROBORATION, NOT A CONDITION:** `markdown_len` 1175 from 1177 source bytes is the **same 2-character delta V6 saw** (1169 from 1171), so the converter behaved identically in production and preview. The row never stalled at `processing` and no error text appeared at any point. | Vercel production built from **`3cd91bd`** (PR #78's squash merge) · observed 2026-08-10T15:43Z · **RETAINED** — Vercel does not reap its deployments (§8.1.1) · Railway ingestion service: **`dep 35cbf150`** — a **deployment-ID prefix, not a commit** — built from `3cd91bd`, which **superseded `6e03e8bd` as a direct consequence of this merge** (empty Watch Paths, register **#55**; **zero files under `services/`** in PR #78's diff). **ARTIFACT REAPED — SCHEDULED, NOT YET TRUE AT TIME OF WRITING**, exactly as V6's cell was written: `35cbf150` was **ALIVE and serving** when this row was taken, and the **next** push to `main` reaps it. **THE IDENTIFICATION IS MACHINE-VERIFIABLE, NOT A DASHBOARD READ:** Railway posts a commit status on `3cd91bd` (context `athletic-miracle - knowflow`) carrying deployment id `35cbf150-9119-462d-afa1-0586899778d8`, and it was observed transitioning `pending` → `success` before the upload; `gh api repos/tornidomaroc-web/knowflow/commits/3cd91bd/status` reproduces it for as long as GitHub retains the status. **DRIFT CHECK PASSED:** `/health` read immediately before the upload was **byte-identical to the pre-merge capture** — `{"ok":true,"embed_provider":"voyage","embed_model":"voyage-3-large","embed_dim":1024,"supabase_configured":true}` — on **all four fields, including `embed_provider` and `embed_model`, which `production-monitor` does not assert (B9-4)**. So the #55 rebuild resolved equivalent bytes on this occasion, and the two-cause hazard did not become concrete |
...
### THE ACCOUNT-IDENTITY FINDING — "the real account" is not a defined term, and the first N11 query returned ZERO ROWS because of it

**Recorded 2026-08-10, from the N11 run. It was our record that was wrong, not the product.**

Q3 was first run with `<UID>` = **`72ebd7b5-dae9-48da-ac53-d58e4533eedf`**, derived exactly as this
protocol suggests — from V4's known real-account document `6c0ff12c`. **It returned zero rows.** The
N11 subject and the N11 document are owned by **`d6fff4eb-6b98-4750-ac7f-2d5001f233f7`**, the live
production session. Both are real production accounts. **The upload was NOT repeated; the finding was
reached by widening the search, which is the correct response and is recorded as the precedent.**

**The defect is in this document, not in the application.** This file says *"your real account"* and
*"the real account"* in §3.1, §2.5, §6.4 and the N1 row, **never once naming a `user_id`** — while it
records three throwaway UUIDs precisely. A designation that resolves to two different principals is
not a designation.

**WHY THIS IS DANGEROUS RATHER THAN UNTIDY, and it is the same lesson §10 already teaches one level
up:** a zero-row Q3 is **indistinguishable from a failed upload** to anyone who has not been told
there are two accounts. §3.1's remedy for a failed N11 is a **Vercel production rollback**. So the
next reader who runs a production check against the wrong UID, gets zero rows, and follows this
document faithfully **rolls back production on a query artefact.** That is a false TRIGGER, arrived at
by obeying the protocol. It is the §10 lesson — *an identifier recorded without stating what it points
to will be read as the wrong kind of thing* — reappearing as an identifier **not recorded at all**.

**Not resolved here, and deliberately so:** which account is "the real account", and whether the
second is a duplicate, a device session, or intended, is **unknown and is not guessed at**. Both UUIDs
are recorded above; neither is blessed. **Ruled to warrant its own register row rather than an
in-place correction — opened as #57 in `PROGRESS.md` §4 by this PR** — because it is a standing
hazard that recurs at *every* future production check (N1 re-runs, **N6 at PR C**, B9-1's two-direction
test), not a one-time false sentence. **The binding rule it opens with: every production check must
record the `user_id` it ran against, in its own §10 cell.** N11's cell above is the first to comply.

### The UI said READY, with a chunk count, before any query ran — and that is a trap, not a convenience

**Recorded 2026-08-10 from the same run.** The production UI displayed `ready` **and `1 chunk`** before
Q3 was executed. Had the run adjudicated on the screen, it would have passed N11 **on `chunk_count`
alone — a column the pipeline WRITES — without ever comparing it to `actual_chunk_rows`, which is what
the chunk table actually contains.**

Q3's `chunk_count = actual_chunk_rows` clause exists precisely because those two can disagree, and the
UI surfaces only the first of them. **A green screen is the pipeline reporting on itself.** This is the
same shape as V9's finding, where a fixture the protocol mandated reached `ready` while being nine
bytes of `not a pdf`: the system's own account of its success is not evidence of success. **No §10 row
may be adjudicated from the UI**, and N11's row above was taken from the raw database row.
...
| Date | Change |
|------|--------|
| 2026-08-10 | **N11 FILLED — PASS, and PR B's sequence is complete.** One production upload (`xilvaroth-n11-20260810.md`, document `bfd6e9d5`) satisfied all five Q3 conditions against Vercel production built from `3cd91bd` and Railway `35cbf150`. **V12 was discharged by the merge itself**, as its CONDITIONAL cell said it would be. **`6e03e8bd`'s `ARTIFACT REAPED — SCHEDULED` marker came TRUE on schedule**: the merge superseded it with `35cbf150` despite **zero files under `services/`**, which is register **#55** confirmed a second time — this time with the **before/after deployment pair and a machine-readable GitHub commit status** rather than a dashboard read alone. **Register #55's mechanism was already empirically confirmed on 2026-08-02 via PR #74; this is corroboration, not a first observation.** **The drift check passed:** `/health` was byte-identical to the pre-merge capture on all four fields. **TWO FINDINGS FROM THE RUN, both recorded below §10's table:** (1) **the account-identity finding** — the first Q3 returned zero rows because *"the real account"* is not a defined term in this file and resolves to two different `user_id`s; **register #57 opened**, and every production check must now record the UID it ran against; (2) **the UI showed `ready` with a chunk count before any query ran**, and adjudicating on it would have passed N11 on a written column without comparing it to `actual_chunk_rows`. **B9-2 CLOSED in this file:** all four `~15 minute` cadence claims (§9.1, §10's preamble, the V1 and N9 evidence cells) corrected to the MEASURED `~68 min daytime / ~113 min full-cycle`. **B9-4 opened** — `production-monitor` asserts `ok`, `embed_dim` and `supabase_configured` but **never `embed_provider` or `embed_model`**, so a model swap under the mutable base tag would pass the monitor silently. Scope: this file, `PROGRESS.md`, `b9-queue.md`. **ZERO files under `src/`. ZERO under `services/`.** |
