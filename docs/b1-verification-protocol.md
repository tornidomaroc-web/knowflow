...
deploying reds within **one tick — MEASURED at ~70 minutes by day and past 3.5 hours overnight, not
the "~15 minutes" this sentence claimed until 2026-08-10 (B9-2)**. That is a real backstop and it is why the
...
and **delivers a measured mean of ~68 min daytime / ~113 min full-cycle** (B9-2; the "every 15
minutes" written here until 2026-08-10 was never delivered). They stopped
...
| V1 | `/health` returns 200 | machine-verified + **CI-observed** | 2026-08-02 14:06:10 | **PASS** — HTTP 200 | `GET /health` → `200` in 0.43s. Re-asserted since 14:59Z by `production-monitor` (PR #75) at a **requested** `*/30` and a **MEASURED mean of ~68 min daytime / ~113 min full-cycle** — **corrected 2026-08-10 from "every 15 min", which this cell asserted and the scheduler never delivered (B9-2)**; first run [30753322454](https://github.com/tornidomaroc-web/knowflow/actions/runs/30753322454) green in 9s. | built from `9e31c22` · observed 2026-08-02T14:06:10Z · **ARTIFACT REAPED** (Railway status: *Removed*) · dep `d7fb32b5` — an 8-hex **deployment-ID prefix, NOT a commit SHA** · deployment live 2026-08-02T14:02Z → 14:59:24Z, so the observation falls inside its lifetime |
