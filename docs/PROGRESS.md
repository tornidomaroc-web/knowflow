...
- **Last updated:** 2026-07-07 (**P3.2 summary UI live-verified + two fixes.** Phase 3 P3.0 (schema + types), P3.1 (summary route + rate-limit wiring), and P3.2 (per-document summary UI) are now **done on the branch** (PR **#16** still **draft**) and **live-verified** on Preview. A live test drove two fixes (branch commit `717ecf5`, rides with PR #16): summaries now follow the **app UI language** (not the document's language) and are stored as **clean plain text** (stray Markdown stripped). Recorded as register **#27** (docs-only; the code lands with PR #16). Prior update 2026-07-05: **Live-DB reconciliation** — audited the live Supabase project against the repo migrations before P3.1 and found it **far behind**: `usage_counters` + `increment_usage()` and several objects had never actually been applied, so the Phase-0 rate limits were **NOT live**; reconciled via a consolidated idempotent script (register **#23** + lesson). Also register **#24** — charge-on-failure rate-limit semantics accepted-and-deferred; PR **#19** landed the repo-wide ESLint config with `ignoreDuringBuilds:true`, restore-build-gating logged as register **#25**; register **#26** records the deliberate "no regenerate path" summary decision.).
- **Active branch:** `feat/phase-3-summaries` (Phase 3, Summaries) — P3.0/P3.1/P3.2 committed (through `717ecf5`), PR **#16** open as **draft**, live-verified on Preview. This `docs/*` branch is a docs-only side-step (register **#27**) landing on `main` ahead of PR #16.
- **Main tip:** `e103834` (PR **#23**, em-dash guard + user-facing dash fixes) on `main`; this docs PR (register **#27**) lands on top.
...
| **3** | Summaries (per-document) | 🔄 **IN PROGRESS** | P3.0 (schema + types), P3.1 (summary route + rate-limit wiring), P3.2 (summary UI) all **done on branch** (through `717ecf5`) and **live-verified** on Preview; PR **#16** still **draft** (`feat/phase-3-summaries`). A live test drove two fixes: summary follows **UI language** + stored as **clean plain text** (register **#27**). Phase row flips to ✅ **DONE** when P3 merges |
